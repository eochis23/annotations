...
/** @typedef {{rgba:number[], pts:Pt[], windowSeq?: number}} Stroke */
...
export class StrokeModel {
    ...
    _cloneStrokes() {
        return this.strokes.map(s => ({
            ...
            windowSeq: s.windowSeq ?? -1,
        }));
    }

    _pushUndoState() {
        this._undoStack.push(this._cloneStrokes());
        while (this._undoStack.length > this._undoCap)
    ...
    beginStroke(x, y, p, windowSeq = -1) {
        this.commitCurrent();
        this.eraserPath = null;
        this._eraserWindowSeq = -1;
        this.current = { rgba: [...this.penRGBA], pts: [{ x, y, p }], windowSeq };
    }

    beginErase(x, y, windowSeq = -1) {
        this.commitCurrent();
        this.current = null;
        this._eraserWindowSeq = windowSeq;
    ...
    _applyEraser() {
        if (!this.eraserPath?.length)
            return;
        const seq = this._eraserWindowSeq;
        this.strokes = this.strokes.filter(s => {
            if (seq >= 0 && (s.windowSeq ?? -1) !== seq)
                return true;
            return !strokeHitByEraser(s, this.eraserPath);
        });
    }
     ...
     * @param {number} width
     * @param {number} height
     * @param {{seq:number,x:number,y:number,w:number,h:number}[]|null} [clipRects] window rects in draw-area coords
     */
    paint(cr, width, height, clipRects = null) {
        cr.save();
        ...
        for (const r of clipRects) {
            ...
            cr.clip();
            for (const s of this.strokes) {
                if ((s.windowSeq ?? -1) === r.seq)
                    this._drawStroke(cr, s);
            }
            if (this.current && (this.current.windowSeq ?? -1) === r.seq)
                this._drawStroke(cr, this.current);
            cr.restore();
        }
        if (this.current && (this.current.windowSeq ?? -1) === -1)
            this._drawStroke(cr, this.current);
        drawEraserPreview();
    }
