...
export class OverlaySession {
    ...
    constructor(extension, strokeModel) {
        ...
        /** @type {{ ax: number, ay: number, rx: number, ry: number }|null} false-dock tablet → stage delta */
        this._penFalseDockRef = null;
    ...
    _dispatchPenCapture(sx1, sy1, fn) {
        try {
            if (this._stagePointInOverlayRoot(sx1, sy1))
                this._lastGoodCapStageXY = [sx1, sy1];
        } catch {
            /* ignore */
        }
    ...
    _penEventLikelyOverDockPalette(event) {
        ...
        for (const xy of sources) {
            if (this._penStagePointDockHit(xy[0], xy[1]))
                return true;
        }
        return false;
    }
    ...
    _penMotionShouldImplicitPress(event) {
        ...
        if (ET.PROXIMITY_IN !== undefined && t === ET.PROXIMITY_IN && eventIsPenDrawDevice(event)) {
            const xy = this._stageCoordsFromEvent(event);
            if (xy && this._penStagePointDockHit(xy[0], xy[1]))
                return false;
    ...
    _penCaptureAcceptsEvent(event) {
        ...
        if (!this._penDown && this._penEventLikelyOverDockPalette(event))
            return false;
    ...
    _onPenStageCaptured(event) {
        ...
        if (this._penCapEntryDbgN < 25 && eventIsPointerButtonOrMotion(event)) {
            ...
            if (xy0 && typeof xy0[0] === 'number' && typeof xy0[1] === 'number') {
                ...
                inDock = this._penStagePointDockHit(xy0[0], xy0[1]);
            }
            // #region agent log
            annoDebug({
                hypothesisId: 'G',
        ...
        if (!inR2 && !proxInGate && !enterPenGate)
            return this._penDown ? Clutter.EVENT_STOP : Clutter.EVENT_PROPAGATE;
        /* Tablet stage coords can sit on the dock AABB while the core pointer stays on canvas.
         * On Wayland `get_pointer()` often does not track the pen (log: px/py frozen at 1439/335
         * while sxEv moves) — remapping every MOTION to the pointer made invisible strokes. Use
         * pointer only as a one-time stage anchor plus tablet deltas vs the first bogus sample. */
        if (this._penDown && eventIsPenDrawDevice(event) && this._penStagePointDockHit(sx, sy)) {
            try {
                const p = global.get_pointer();
                if (
                    Array.isArray(p) &&
                    ...
                    const dx = sx - p[0];
                    const dy = sy - p[1];
                    if (dx * dx + dy * dy > TABLET_DOCK_COORD_SLACK2) {
                        if (!this._penFalseDockRef)
                            this._penFalseDockRef = { ax: p[0], ay: p[1], rx: sx, ry: sy };
                        const ref = this._penFalseDockRef;
                        sx1 = ref.ax + (sx - ref.rx);
                        sy1 = ref.ay + (sy - ref.ry);
                        inR2 = this._stagePointInOverlayRoot(sx1, sy1);
                        if (!inR2) {
                        ...
                        if (this._penDockPtrOverrideDbgN < 20) {
                            ...
                            annoDebug({
                                hypothesisId: 'I',
                                location: 'overlaySession.js:_onPenStageCaptured',
                                message: 'false-dock tablet coords → stage via anchor+delta',
                                data: { sxEv: sx, syEv: sy, ax: ref.ax, ay: ref.ay, sx1, sy1 },
                            });
                            // #endregion
                        }
                    } else {
        ...
        if (inR2 && this._penStagePointDockHit(sx1, sy1)) {
            if (!this._penDown)
                return Clutter.EVENT_PROPAGATE;
