...
/**
 * Some Mutter/GIR stacks report stylus tip press as numeric 16 while
 * `Clutter.EventType.BUTTON_PRESS` is a different constant — capture must still dispatch `_onPress`.
 */
function isDiscretePressEventType(t, ET) {
    if (t == null || t === undefined || !ET)
        return false;
    return t === ET.BUTTON_PRESS || t === ET.TOUCH_BEGIN || t === 16;
}
...
export class OverlaySession {
    ...
    _bumpDockActivity() {
        ...
        this._dockShrinkSrc = GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, 5, () => {
            ...
            if (this._dock && this._root?.visible) {
                ...
                this._dock.set_scale(0.48, 0.48);
            }
    ...
    _dockExpandIfShrunk() {
        if (!this._dockShrunk || !this._dock)
            return;
        this._dockShrunk = false;
        this._dock.set_scale(0.66, 0.66);
    }
    ...
    _onPenStageCaptured(event) {
        ...
        if (isDiscretePressEventType(t, ET))
    ...
    _createDock() {
        ...
        for (let i = 0; i < PEN_COLORS.length; i++) {
            ...
            btn.set_style(`background-color: ${rgba}; min-width: 20px; min-height: 20px; border-radius: 75px;`);
            const idx = i;
            btn.connect('clicked', () => {
    ...
    _onPress(actor, event) {
        try {
            ...
            const isDiscretePress = ETp && isDiscretePressEventType(tPress, ETp);
                ...
                try {
                    ...
                    if (
                        ...
                        if (
                            ...
                            this._penDeferredAnchorStageXY = [xy[0], xy[1]];
                    }
                } catch {
                    this._penDeferredAnchorStageXY = null;
                }
        ...
        try {
            ...
            if (!this._penDown) {
                ...
                if (this._penAwaitInkAfterRange && eventIsPenDrawDevice(event)) {
                    ...
                    if (motionish && this._penDeferredAnchorStageXY) {
                        let mx = null;
                        let my = null;
                        const cap = this._penCapStageXYPair;
                        if (
                            cap &&
                            cap.length >= 2 &&
                            typeof cap[0] === 'number' &&
