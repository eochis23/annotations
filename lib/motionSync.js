...
/**
 * Deterministic pseudo-random grey8 (avoid low-entropy patterns that create
...
export class MotionSyncService {
    ...
    runSyntheticSelfTest(log) {
        ...
        runMotionMatchAsync(helper, w, h, prevPath, curPath, this._cancellable, (res, err) => {
            ...
            log(`synthetic result dx=${res.dx} dy=${res.dy} c=${res.c.toFixed(3)} (threshold ${thr}, geomOk=${geomOk}; expected dx≈${tdx} dy≈${tdy})`);
            if (geomOk || res.c >= thr) {
                this._model.translateAll(res.dx, res.dy);
                log('applied translateAll to stroke model');
            } else if (!geomOk) {
                log('no translate applied (confidence too low and shift not near expected)');
            }
        });
    }
