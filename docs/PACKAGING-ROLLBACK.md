...
On **Arch Linux**, refer to the official `extra/mutter` and `extra/gnome-shell` `PKGBUILD` files: copy them into a personal repo, add `patches/*.patch` to `source` and apply in `prepare()`.
...
`scripts/rollback-hint.sh` prints this file path and cache locations (no system changes).
