...
2. Build native helper (from repository root): `cd native && meson setup build && meson compile -C build`
...
Or use `make pack` from the repository root [`Makefile`](../../Makefile).
